...
void print_array(heap_value arr[], size_t size) {
...
bool is_heap(heap_value arr[], size_t i, size_t size) {
  size_t l = 2 * i + 1;
  size_t r = 2 * i + 2;
  if (r >= size) {
    return true;
...
// Helper function:
// Checks whether the given priority queue is in order.
bool is_queue_heap(const priority_queue *q, size_t i) {
  size_t l = 2 * i + 1;
  size_t r = 2 * i + 2;
  if (r >= q->size) {
    return true;
  }
  if (queue_compare(q->arr[i], q->arr[l]) >= 0 && queue_compare(q->arr[i], q->arr[r]) >= 0) {
    return is_queue_heap(q, l) && is_queue_heap(q, r);
  }
  return false;
}
...
bool is_descending(heap_value arr[], size_t size) {
...
int main() {
  ...
  heap_value nums[] = {8, 6, 7, 5, 3, 0, 9, 80, 60, 70, 50, 30, 0, 90};
